...
Figure 1 depicts the overall numbers of refugees in Toronto's shelters between 2018 and 2022, organized by gender (including those who identified as female, male, and nonbinary).
...
#Create annual tallies of refugees per year by gender
...
#Create Table 1: Chart of total refugees per year by gender 
Graph1 <- cbind(Male_Refugees_by_year, Female_Refugees_by_year, Nonbinary_Refugees_by_year)

#Graph data
Graph1|>
  ggplot(aes(x = year, y = Graph1 )) +
  geom_line() +
  geom_point() +
  theme_classic()
  xlab = "Year" +
...
```
In addition to gender, this report will also look at age as a variable. Figure 2 looks at how five age groups are represented among refugees in Toronto's shelters.

```{r}

#Create annual tallies of ages per year 
AgeUnder16Total <- Year_Demographic %>% group_by(year)%>% tally(sum(ageunder16))

Age16to24Total <- Year_Demographic %>% group_by(year)%>% tally(sum(`age16-24`))

Age25to44Total <- Year_Demographic %>% group_by(year)%>% tally(sum(`age25-44`))

Age45to64Total <- Year_Demographic %>% group_by(year)%>% tally(sum(`age45-64`))

Age65OverTotal <- Year_Demographic %>% group_by(year)%>% tally(sum(`age65over`))

#Create dataset with these variables
Graph2 <- cbind(year,AgeUnder16Total, Age16to24Total, Age25to44Total, Age45to64Total, Age65OverTotal)

#Graph data
Graph2 |>
  as_tibble() |>
  ggplot(mapping = aes(x = Graph2, y = Years)) +
  geom_bar() +
  theme_classic()
...
```{r}
#Create annual tallies of the amount of actively homeless refugees between 2018 and 2022
Actively_Homeless2018 <- sum(Year_Housing[which(Year_Housing$year == "2018"),4])

Actively_Homeless2019 <- sum(Year_Demographic[which(Year_Demographic$year == "2019"),4])

Actively_Homeless2020 <- sum(Year_Demographic[which(Year_Demographic$year == "2020"),4])

Actively_Homeless2021 <- sum(Year_Demographic[which(Year_Demographic$year == "2021"),4])

Actively_Homeless2022 <- sum(Year_Demographic[which(Year_Demographic$year == "2022"),4])

Actively_Homeless_Total <- cbind(Actively_Homeless2018, Actively_Homeless2019, Actively_Homeless2020, Actively_Homeless2021, Actively_Homeless2022)

#Create dataset and graph data
Actively_Homeless_Total |>
  ggplot(mapping = aes(x = Actively_Homeless_Total, y = year)) +
  geom_line() +
  geom_point() +
  xlab(x = "Actively Homeless Per Year") +
  ylab(y = "Year")
...
```{r}
#Create annual tallies of the amount of refugees who are new to the shelter system between 2018 and 2022

Newly_Identified2018 <- sum(Year_Housing[which(Year_Housing$year == "2018"),5])

Newly_Identified2019 <- sum(Year_Housing[which(Year_Housing$year == "2019"),5])

Newly_Identified2020 <- sum(Year_Housing[which(Year_Housing$year == "2020"),5])

Newly_Identified2021 <- sum(Year_Housing[which(Year_Housing$year == "2021"),5])

Newly_Identified2022 <- sum(Year_Housing[which(Year_Housing$year == "2022"),5])

#Combine all data
Newly_Identified_Total <- cbind(Newly_Identified2018, Newly_Identified2019, Newly_Identified2020, Newly_Identified2021, Newly_Identified2022)

#Create dataset and graph data
Newly_Identified_Total|>
  ggplot(mapping = aes(x = Newly_Identified_Total, y = year)) +
  geom_line() +
  geom_point() +
  xlab(x = "Newly Identified Refugees Per Year") +
  ylab(y = "Year")
